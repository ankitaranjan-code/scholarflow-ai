...
export default function ChatInterface({ initialMessages, student, studentId = 1 }) {
  ...
  const handleApplyRoutine = async (routine) => {
    try {
      const routines = await api.getGamificationRoutines(studentId);
      const routineData = {
        name: routine.name || "AI Suggested Routine",
        description: "Generated by AI companion",
        tasks: routine.tasks
      };

      if (routines.length > 0) {
        await api.updateGamificationRoutine(studentId, routines[0].id, routineData);
      } else {
        await api.createGamificationRoutine(studentId, routineData);
      }
      
      alert('Routine applied! Check your dashboard. 🚀');
    } catch (err) {
      alert('Failed to apply routine: ' + err.message);
    }
  };
    ...
    <div className="chat-container animate-fade-in">
      ...
      <div className="chat-body">
        ...
        <div className="messages-scroll">
          {messages.map((msg) => {
            const routineMatch = msg.content.match(/```json\n([\s\S]*?)\n```/);
            let suggestedRoutine = null;
            if (routineMatch) {
              try {
                const parsed = JSON.parse(routineMatch[1]);
                if (parsed.type === 'routine_suggestion') {
                  suggestedRoutine = parsed;
                }
              } catch (e) {}
            }
            ...
            return (
              <div key={msg.id} className="message-container">
                <div className={`message-row msg-${msg.role}`}>
                  ...
                  <div className={`msg-bubble ${msg.role === 'user' ? 'msg-user-bubble' : 'msg-ai-bubble'}`}>
                    <p className="msg-text" dangerouslySetInnerHTML={{
                      __html: msg.content
                        .replace(/```json\n[\s\S]*?\n```/g, '') // Hide the JSON block from text
                ...
                {suggestedRoutine && (
                  <div className="ai-action-suggestion animate-fade-in" style={{ marginLeft: '3rem', marginBottom: '1rem' }}>
                    <div className="glass-card" style={{ padding: '1rem', border: '1px solid var(--primary)' }}>
                      <p className="t-body" style={{ fontWeight: 600, marginBottom: '0.5rem' }}>✨ New Routine Suggestion</p>
                      <p className="t-small text-muted" style={{ marginBottom: '1rem' }}>
                        The AI has generated a personalized {suggestedRoutine.tasks.length}-task routine based on your conversation.
                      </p>
                      <button 
                        className="btn btn-primary btn-small" 
                        onClick={() => handleApplyRoutine(suggestedRoutine)}
                        style={{ width: '100%' }}
                      >
                        Apply to Dashboard
                      </button>
